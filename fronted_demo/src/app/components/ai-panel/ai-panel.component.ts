...
export class AiPanelComponent implements OnInit, OnDestroy {
  ...
  onExport(): void {
    const exportable = this.messages.filter(m => m.role === 'user' || m.role === 'assistant');
    if (exportable.length === 0) {
      this.message.warning('当前对话无内容可导出');
      return;
    }
    this.exportService.exportConversation(exportable, 'AI对话');
    this.message.success('对话已导出为 Markdown 文件');
  }
  ...
  onMessageSent(text: string): void {
    ...
    const dashboardContext = this.aiContextService.getFullContext();
    const hasContext = Object.keys(dashboardContext).length > 0;

    const { stream$, abort, sessionId$ } = this.aiService.chatStream({
      message: text,
      session_id: this.currentSessionId || undefined,
      context: hasContext ? dashboardContext : undefined,
    });
    this.currentStreamAbort = abort;

    this.subscription.add(sessionId$.subscribe(id => { this.currentSessionId = id; this.loadSessions(); }));
    ...
    this.streamSub = stream$.subscribe({
      ...
      error: (err: any) => {
        this.isWaiting = false; this.currentStreamAbort = null; this.streamSub = null;
      ...
      complete: () => { this.isWaiting = false; this.currentStreamAbort = null; this.streamSub = null; },
    });
  }

  clearChat(): void { this.onNewChat(); this.aiPanelService.clearContext(); }
